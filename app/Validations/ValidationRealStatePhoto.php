...
class ValidationRealStatePhoto
{
    private $erros = [];

    public function validateRealStatePhoto($data, $model = null, $action = null, $imagem = null)
    {
    ...
    public function validateIdRealStatePhoto($photoId, $realStateId, $model)
    {
        $photo = $model->where('real_state_id', $realStateId)->where('id', $photoId)->first();

        if (!is_numeric($photoId) || !is_numeric($realStateId) || $photo === null) {
            $this->erros['error-id'] = "Foto ou Thumb do Imóvel não Encontrada!";
        }

        return $this->erros;
    }

    public function validateIdRealPhotoState($photoId, $model)
    {
        $photo = $model->where('id', $photoId)->first();

        if (!is_numeric($photoId) || $photo === null) {
            $this->erros['error-id'] = "Foto do Imóvel não Encontrada!";
        }
